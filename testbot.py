...
def handle_start_help(message):
    if (baza.get_data(str(message.chat.id) + 'name')):
        ...
        bot.send_message(message.chat.id, "Привіт!"
                                          "\n Я, бот, який вміє рандомити🤫, класно правда?"
                                          "\n Мені потрібні такі, як ти"
...
def user_entering_name(message):
    ...
    bot.send_message(message.chat.id, f"Дальше тобі знадобиться /help")
...
def random_stats_name(message):
     ...
     if message.text.lower() == "breed":
        ...
        bot.send_message(message.chat.id,f"Bred:  {breedgroup},"
                                        f"\nLife span:  {life},"
                                        f"\nBred for:  {bred}"
        )
